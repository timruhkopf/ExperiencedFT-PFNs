...
class TransformerModel(nn.Module):
    ...
    def get_attention_outputs(self):
        ...
        for l in self.transformer_encoder.layers:
            attention_outputs.append(l.attn_output)
        return torch.stack(attention_outputs)

    def get_hidden_state_outputs(self):
        hidden_state_outputs = []
        for l in self.transformer_encoder.layers:
            hidden_state_outputs.append(l.hidden_state)
        return torch.stack(hidden_state_outputs)
    ...
    def forward(self, *args, layers_info = None, **kwargs):
        """
        ...
        if len(args) == 3:
            ...
            return self._forward(layers_info = layers_info, *args, **kwargs)

    def _forward(self, src, src_mask=None, single_eval_pos=None, only_return_standard_out=True, layers_info=None):
        ...
        output = self.transformer_encoder(src, src_mask, layers_info=layers_info)
...
class TransformerEncoderDiffInit(Module):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, layers_info=None) -> Tensor:
