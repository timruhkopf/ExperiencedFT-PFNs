...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, task_info = {}) -> Tensor:
        ...
        f_attn = task_info.get("f_attn", None)
        f_hidden_state = task_info.get("f_hidden_state", None)
        f_support_attn = task_info.get("f_support_attn", None)
        ...
        if isinstance(src_mask, tuple):
            ...
            if f_support_attn is not None:
                src_right = f_support_attn(src_right, src_[single_eval_position:], self.self_attn)
        ...
        self.hidden_state = src.clone()
        if f_hidden_state is not None:
            src = f_hidden_state(src)
        return src