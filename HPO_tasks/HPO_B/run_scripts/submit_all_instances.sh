...
policies=("PFNs4BO-HEBO-PI" "ourPFNs") #"ourPFNs")
...
for ((i=0; i<search_space_ids_length; i++)); do
	...
	echo "Running dataset: $search_space_id, policy: $policy"
        ...
        output=$(eval sbatch --partition=$partition --error=$error_path --output=$output_path runoncluster.sh $search_space_id $policy)
