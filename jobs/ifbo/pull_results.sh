...
EXPERIMENT_NAME=EQW-pfnimpute-cv-flip
EXPERIMENT_GROUP=synthetic-tasks
...
python main_ifbo.py \
  ...
  experiment_name=$EXPERIMENT_NAME \
  benchmark=synthetic  \
  seed=0,1,2,3,4,6,7,8,9 \
  +algorithm=ifbo-pfnimpute-cv \
    algoname=$EXPERIMENT_NAME \
  split_seed=0 \
  +fold=0 \
  +target_idx=0 \
  device=cpu  \
...
#  +benchmark.cls.data_path=$BIGWORK/ExperiencedFT-PFNs/data/synthetic_tabular \
...
--keys "[\"experiment_name\",\"algoname\",\"benchmark.meta.name\",\"split_seed\"]" \
