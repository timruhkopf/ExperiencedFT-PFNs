...
def aggregate_anytime(df):
    agg = (
        df.groupby(['benchmark.meta.name', 'algorithm.surrogate_model.meta.name', 'step'])
          .agg(mean_anytime=('anytime_performance', 'mean'),
               std_anytime=('anytime_performance', 'std'))
          .reset_index()
    )
    return agg
...
def plot_anytime_performance(agg_df):
    ...
    def plot_with_error(data, color, label, **kwargs):
        plt.plot(data['step'], data['mean_anytime'], label=label, color=color)
        plt.fill_between(
            data['step'],
            data['mean_anytime'] - data['std_anytime'],
            data['mean_anytime'] + data['std_anytime'],
            color=color,
            alpha=0.2
        )
...
if __name__ == '__main__':
    file = Path('/home/ruhkopf/PycharmProjects/ExperiencedFT-PFNs/kisski_results/neps_output.csv')
    df = pd.read_csv(file)
